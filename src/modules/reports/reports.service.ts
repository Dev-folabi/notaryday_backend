...
export class ReportsService {
  ...
  async taxPdf(userId: string, from: string, to: string): Promise<Buffer> {
    ...
    return new Promise<Buffer>((resolve, reject) => {
      ...
      // Printable bottom boundary (page height - bottom margin) is ~142pt
      // above us; PDFKit auto-breaks any text placed beyond that point, which
      // is what caused phantom footer-only pages. Keep all content and the
      // footer below the boundary.
      // Content must stop ~128pt above the bottom edge so the footer lives in
      // the space between the content floor and the printable boundary.
      ...
      const drawHeader = () => {
        ...
        doc
          .fontSize(16)
          .font('Helvetica-Bold')
          .text('Schedule C, Notary Income Summary', 50, 28);
        doc
          .fontSize(10)
          .font('Helvetica')
          .fillColor('#B8C7DD')
